  ...
  threads.forEach(thread => {
    ...
    messages.forEach(message => {
      if (message.getDate() >= since) {
        ...
        const articles = extractArticles(htmlBody);

        emails.push({
          subject: message.getSubject(),
          body: message.getPlainBody().slice(0, 1000),
          articles: articles,
        });
      }
    });
...
// ===== HTMLから記事タイトルとURLを抽出 =====
function extractArticles(htmlBody) {
  const articles = [];

  // まずGoogleアラート形式（JSONのscriptタグ）を試みる
  const scriptMatch = htmlBody.match(
    /<script[^>]+data-scope="inboxmarkup"[^>]*>([\s\S]*?)<\/script>/
  );
  ...
  if (scriptMatch) {
    ...
    try {
      ...
      cards.forEach(card => {
        ...
        widgets.forEach(widget => {
          if (widget.type === 'LINK' && widget.title && widget.url) {
            const urlMatch = widget.url.match(/[?&]url=([^&]+)/);
            const actualUrl = urlMatch
    ...
    // Scholar Inbox形式（aタグ）
    const regex = /<a[^>]+href="(https?:\/\/[^"]+)"[^>]*>([^<]{10,})<\/a>/gi;
    let match;
    ...
    while ((match = regex.exec(htmlBody)) !== null) {
      ...
      if (
        ...
        !url.includes('scholar-inbox.com') &&
        title.length > 10
      ) {
...
function buildNotionBlocks(summary, emails) {
  ...
  emails.forEach(email => {
    if (email.articles && email.articles.length > 0) {
      // 記事タイトルとURLが取得できた場合
      email.articles.forEach(article => {
        blocks.push({
          object: 'block',
          type: 'bulleted_list_item',
          bulleted_list_item: {
